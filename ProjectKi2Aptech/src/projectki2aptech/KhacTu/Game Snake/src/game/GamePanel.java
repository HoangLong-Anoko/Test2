...
public class GamePanel extends JPanel implements Runnable, KeyListener {
	...
	private Enity head,apple;
	private ArrayList<Enity> snake;
	private int score;
	private int level;
	...
	private void setUplevel() {
		snake = new ArrayList<Enity>();
		head = new Enity(SIZE);
		head.setPosition(WIDTH / 2, HEIGHT / 2);
		snake.add(head);
		for(int i = 1; i < 3; i++) {
			Enity e = new Enity(SIZE);
			e.setPosition(head.getX() + (i * SIZE), head.getY());
			snake.add(e);
		}
		apple = new Enity(SIZE);
		setApple();
		score = 0;
		gameover = false;
		level = 1;
	...
	public void render(Graphics2D g2d) {
		...
		apple.render(g2d);
		if(gameover) {
			g2d.drawString("GameOver!!!!!", 500, 350);
		}
		
		g2d.setColor(Color.WHITE);
		g2d.drawString(" Score : " + score + " Level : " + level, 20, 20);
		if(dx == 0 && dy == 0) {
			g2d.drawString("Ready!!!!!", 500, 350);
		}
	}
}